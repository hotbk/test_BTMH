...
with d_date as(
  ...
  FROM UNNEST(
	...
	CROSS JOIN (select distinct warehouse_raw from `btmh-airflow-dbt-lab-2026`.`btmh_dwh_dim`.`d_store`) AS store_code
)
...
,hang_report as(
  ...
  from `btmh-airflow-dbt-lab-2026`.`btmh_dwh_dim`.`d_hang_agg`
)
...
,khach_dat_raw as(
  ...
  from `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_dat_coc` dc left join hang_report hr on dc.ID_Hang = hr.ID
															left join `btmh-airflow-dbt-lab-2026`.`btmh_dwh_dim`.`d_store` s on dc.nguon = s.company 
...
,b2c_raw as(
  ...
  from `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_doanh_thu` dt  left join hang_report hr on dt.ID_Hang = hr.ID
