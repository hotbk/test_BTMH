...
kcs as (
  select *
  from `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_kcs_merged`
),
...
n as (
  select id, ma_nhom
  from `btmh-airflow-dbt-lab-2026`.`btmh_dwh_dim`.`d_nhom`
),
...
(
  case
    when kcs.ngay_kcs_k1 is null then null

    -- epoch-like numerics
    when regexp_contains(cast(kcs.ngay_kcs_k1 as string), r'^\d{19}$') then timestamp_micros(div(cast(kcs.ngay_kcs_k1 as int64), 1000))
    when regexp_contains(cast(kcs.ngay_kcs_k1 as string), r'^\d{16}$') then timestamp_micros(cast(kcs.ngay_kcs_k1 as int64))
    when regexp_contains(cast(kcs.ngay_kcs_k1 as string), r'^\d{13}$') then timestamp_millis(cast(kcs.ngay_kcs_k1 as int64))
    when regexp_contains(cast(kcs.ngay_kcs_k1 as string), r'^\d{10}$') then timestamp_seconds(cast(kcs.ngay_kcs_k1 as int64))

    -- yyyymmddhhmmss as int/string
    when regexp_contains(cast(kcs.ngay_kcs_k1 as string), r'^\d{14}$') then timestamp(
      parse_datetime('%Y%m%d%H%M%S', cast(kcs.ngay_kcs_k1 as string)),
      "Asia/Ho_Chi_Minh"
    )

    -- yyyymmdd as int/string
    when regexp_contains(cast(kcs.ngay_kcs_k1 as string), r'^\d{8}$') then timestamp(
      datetime(parse_date('%Y%m%d', cast(kcs.ngay_kcs_k1 as string))),
      "Asia/Ho_Chi_Minh"
    )

    -- Fall back to BigQuery's parser (handles ISO strings); timezone only applies if the string has no zone.
    else timestamp(cast(kcs.ngay_kcs_k1 as string), "Asia/Ho_Chi_Minh")
  end
)
) as report_date,
...
tsl_src as (
  select *
  from `btmh-airflow-dbt-lab-2026`.`{{ env_var('ggs_bq_dataset', 'btmh_stg_ggs') }}`.`ggs_trang_suc_loi`
),
...
(
  case
    when tsl.ngay_kiem_tra is null then null

    -- epoch-like numerics
    when regexp_contains(cast(tsl.ngay_kiem_tra as string), r'^\d{19}$') then timestamp_micros(div(cast(tsl.ngay_kiem_tra as int64), 1000))
    when regexp_contains(cast(tsl.ngay_kiem_tra as string), r'^\d{16}$') then timestamp_micros(cast(tsl.ngay_kiem_tra as int64))
    when regexp_contains(cast(tsl.ngay_kiem_tra as string), r'^\d{13}$') then timestamp_millis(cast(tsl.ngay_kiem_tra as int64))
    when regexp_contains(cast(tsl.ngay_kiem_tra as string), r'^\d{10}$') then timestamp_seconds(cast(tsl.ngay_kiem_tra as int64))

    -- yyyymmddhhmmss as int/string
    when regexp_contains(cast(tsl.ngay_kiem_tra as string), r'^\d{14}$') then timestamp(
      parse_datetime('%Y%m%d%H%M%S', cast(tsl.ngay_kiem_tra as string)),
      "Asia/Ho_Chi_Minh"
    )

    -- yyyymmdd as int/string
    when regexp_contains(cast(tsl.ngay_kiem_tra as string), r'^\d{8}$') then timestamp(
      datetime(parse_date('%Y%m%d', cast(tsl.ngay_kiem_tra as string))),
      "Asia/Ho_Chi_Minh"
    )

    -- Fall back to BigQuery's parser (handles ISO strings); timezone only applies if the string has no zone.
    else timestamp(cast(tsl.ngay_kiem_tra as string), "Asia/Ho_Chi_Minh")
  end
)
) as report_date,
...
ttl_src as (
  select *
  from `btmh-airflow-dbt-lab-2026`.`{{ env_var('ggs_bq_dataset', 'btmh_stg_ggs') }}`.`ggs_tich_tru_loi`
),
...
(
  case
    when ttl.ngay_kiem_tra is null then null

    -- epoch-like numerics
    when regexp_contains(cast(ttl.ngay_kiem_tra as string), r'^\d{19}$') then timestamp_micros(div(cast(ttl.ngay_kiem_tra as int64), 1000))
    when regexp_contains(cast(ttl.ngay_kiem_tra as string), r'^\d{16}$') then timestamp_micros(cast(ttl.ngay_kiem_tra as int64))
    when regexp_contains(cast(ttl.ngay_kiem_tra as string), r'^\d{13}$') then timestamp_millis(cast(ttl.ngay_kiem_tra as int64))
    when regexp_contains(cast(ttl.ngay_kiem_tra as string), r'^\d{10}$') then timestamp_seconds(cast(ttl.ngay_kiem_tra as int64))

    -- yyyymmddhhmmss as int/string
    when regexp_contains(cast(ttl.ngay_kiem_tra as string), r'^\d{14}$') then timestamp(
      parse_datetime('%Y%m%d%H%M%S', cast(ttl.ngay_kiem_tra as string)),
      "Asia/Ho_Chi_Minh"
    )

    -- yyyymmdd as int/string
    when regexp_contains(cast(ttl.ngay_kiem_tra as string), r'^\d{8}$') then timestamp(
      datetime(parse_date('%Y%m%d', cast(ttl.ngay_kiem_tra as string))),
      "Asia/Ho_Chi_Minh"
    )

    -- Fall back to BigQuery's parser (handles ISO strings); timezone only applies if the string has no zone.
    else timestamp(cast(ttl.ngay_kiem_tra as string), "Asia/Ho_Chi_Minh")
  end
)
) as report_date,
