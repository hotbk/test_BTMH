...
with df_xlsx as (
  ...
  from `btmh-airflow-dbt-lab-2026`.`{{ env_var('ggs_bq_dataset', 'btmh_stg_ggs') }}`.`xlsx_ke_hoach_kinh_doanh`
),
...
df_ggs as (
  select
    ...
    safe_cast(Doanh_thu_KH_ngay as float64) as Doanh_thu_KH_ngay,
    safe_cast(Loi_nhuan_KH_ngay as float64) as Loi_nhuan_KH_ngay
  from `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_ggs_ke_hoach_kinh_doanh`
