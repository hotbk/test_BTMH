...
htk_base AS (
    ...
    FROM `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_hang_ton_kho`
...
nx_movements AS (
    ...
    LEFT JOIN `btmh-airflow-dbt-lab-2026`.`btmh_dwh_dim`.`d_hach_toan` ht
        ON nx.Nguon = ht.Nguon
       AND SAFE_CAST(nx.ID_Nx AS INT64) = SAFE_CAST(ht.ID AS INT64)
    WHERE DATE(nx.Ngay) BETWEEN DATE_TRUNC(DATE_SUB(
(
  
    current_date()
  
)
, INTERVAL 1 DAY), MONTH) AND 
(
...
dc_movements AS (
    ...
    FROM (
        ...
        FROM `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_dieu_chuyen`
        WHERE DATE(Ngay) BETWEEN DATE_TRUNC(DATE_SUB(
(
...
sales_movements AS (
    ...
    FROM `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_doanh_thu`
    WHERE ID_Dv >= 0
      AND DATE(Ngay_PhieuThu) BETWEEN DATE_TRUNC(DATE_SUB(
(
...
all_days AS (
    SELECT k.nguon, k.ma_kho, k.id_hang, 
(
...
so_ban AS (
    SELECT
        ...
        ID_Hang,
                DATE(Ngay_PhieuThu) as ngay,
                SUM(CASE WHEN DATE(Ngay_PhieuThu) = 
(
  
    current_date()
  
)
 THEN So_Luong ELSE 0 END) AS so_luong_ban_today,
                SUM(CASE WHEN DATE(Ngay_PhieuThu) = 
(
 ...
 THEN Tien_PhieuThu1 ELSE 0 END) AS doanh_thu_today,
                SUM(CASE WHEN DATE(Ngay_PhieuThu) = 
(
 ...
 THEN Gia_Von ELSE 0 END) AS gia_von_today,
        
                SUM(CASE WHEN DATE(Ngay_PhieuThu) = DATE_SUB(
(
  
    current_date()
  
)
, INTERVAL 1 DAY) THEN So_Luong ELSE 0 END) AS so_luong_ban_yesterday,
...
, INTERVAL 3 DAY) 
                                    AND DATE(Ngay_PhieuThu) < 
(
  
    current_date()
  
)
 THEN So_Luong ELSE 0 END) AS so_luong_ban_3d,
...
(
 ...
 THEN So_Luong ELSE 0 END) AS so_luong_ban_7d
    FROM `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_doanh_thu`
    WHERE ID_Dv >= 0
            AND DATE(Ngay_PhieuThu) BETWEEN DATE_SUB(
(
...
dp as(
    ...
    from `btmh-airflow-dbt-lab-2026`.`btmh_dwh_mart`.`v_gold_deposit`
    where 1=1
    group by 1,2,3
),
...
final AS (
    ...
    LEFT JOIN `btmh-airflow-dbt-lab-2026`.`btmh_dwh_dim`.`d_hang_1_agg` h
