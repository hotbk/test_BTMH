...
nx_movements AS (
    ...
    LEFT JOIN `btmh-airflow-dbt-lab-2026`.`btmh_dwh_dim`.`d_hach_toan` ht ON nx.Nguon = ht.Nguon AND SAFE_CAST(nx.ID_Nx AS INT64) = SAFE_CAST(ht.ID AS INT64)
...
tf AS (
    ...
    FROM (
        ...
        FROM `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_dieu_chuyen` WHERE DATE(Ngay) = DATE(CURRENT_DATE())
        UNION ALL
...
ie AS (
    ...
    FROM `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_nhap_xuat` nx
    LEFT JOIN `btmh-airflow-dbt-lab-2026`.`btmh_dwh_dim`.`d_hach_toan` ht ON nx.ID_Nx = ht.ID AND nx.Nguon = ht.Nguon
...
final AS (
    ...
    LEFT JOIN `btmh-airflow-dbt-lab-2026`.`btmh_dwh_dim`.`d_hang_1_agg` h ON tk.nguon = h.Ma_vung AND tk.id_hang = CAST(h.ID AS STRING)
)
...
from final2 f
left join `btmh-airflow-dbt-lab-2026`.`btmh_dwh_mart`.`r_exchange_rate` r on f.Ten_Hang = r.Ten_Hang
where 1=1
-- and Ma_Hang = 'NL06'
