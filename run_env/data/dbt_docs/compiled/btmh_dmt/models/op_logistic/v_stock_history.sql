...
WITH base AS (
    SELECT
        ...
        so_luong_ton,
        gia_tri_ton
    FROM `btmh-airflow-dbt-lab-2026`.`btmh_dwh_fact`.`f_ton_kho`
    WHERE ngay >= DATE_SUB(date('2024-01-01'), INTERVAL 1 DAY)
),
...
all_days AS (
    ...
    CROSS JOIN (
        SELECT ngay
        FROM UNNEST(
            GENERATE_DATE_ARRAY(date('2024-01-01'), 
(
...
final AS (
    ...
    FROM ton_kho tk
    LEFT JOIN `btmh-airflow-dbt-lab-2026`.`btmh_dwh_mart`.`v_import_export` ie
        ON  tk.nguon   = ie.Nguon
        AND tk.ma_kho  = ie.Ma_Kho
        AND tk.id_hang = CAST(ie.ID_Hang AS STRING)
        AND tk.ngay    = ie.ngay
    LEFT JOIN `btmh-airflow-dbt-lab-2026`.`btmh_dwh_mart`.`v_transfer` tf
