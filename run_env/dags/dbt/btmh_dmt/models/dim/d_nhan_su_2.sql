{{
  config(
    ...
    cluster_by=['personnel_id', 'code']
  )
}}

with params as (
  select {{ btmh_run_date() }} as run_date
),

src_ls as (
  select *
  from {{ source('oneoffice', 'ns_lich_su_cong_viec') }}
  where dt = (select run_date from params)
),

src_ns as (
  select *
  from {{ source('oneoffice', 'ns_ho_so_nhan_su') }}
  where dt = (select run_date from params)
),
...
joined_raw as (
  select
    ...
    case
      when cast(ls.job_status as string) in ('Làm việc', 'Đang làm việc') then 1
...
filtered_input as (
  ...
  where date_from_parsed is not null
    ...
    and not (
      cast(type as string) = 'Hệ thống'
      and department_main is null
      and department_id is null
      and company_name is null
      and cast(job_status as string) in ('Đang làm việc', 'Làm việc')
      and job_contract is null
      and contract_code is null
    )
),

-- Deduplicate within day: partition by personnel_id/code/yyyy-mm-dd
-- Keep latest record by (date_from desc, ls.ID desc)
ranked_day as (
  select
    * except(rn_dedup)
  from (
    select
      ...
      row_number() over (
        partition by
          cast(personnel_id as string),
          cast(code as string),
          extract(year from date_from_parsed),
          extract(month from date_from_parsed),
          extract(day from date_from_parsed)
        order by date_from_parsed desc, ID desc
      ) as rn_dedup
    from filtered_input f
  )
  where rn_dedup = 1
),
...
filled as (
  select
    ...
    case
      ...
      when extract(year from c.date_from) = extract(year from c.next_date_from)
        and extract(month from c.date_from) = extract(month from c.next_date_from)
        then c.next_date_from
      else coalesce(
