...
with src as (
	...
	from {{ source(var('d_khach_hang_schema') | lower, 'stg_khach_hang') }}

	{% if is_incremental() %}
	where cast(coalesce(ngay_sua, ngay_tao) as timestamp) >= (
		select coalesce(max(UpdateTime), timestamp('1970-01-01')) from {{ this }}
	)
	{% endif %}
),
