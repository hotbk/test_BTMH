{{
  config(
    ...
    partition_by={"field": "Thang", "data_type": "date"},
...
with params as (
  select
    ...
    date_trunc({{ btmh_run_date() }}, month) as start_of_month,
    date_diff({{ btmh_run_date() }}, date_trunc({{ btmh_run_date() }}, month), day) as days_diff,
    safe_divide(
      cast(date_diff({{ btmh_run_date() }}, date_trunc({{ btmh_run_date() }}, month), day) as bignumeric),
      cast((date_diff({{ btmh_run_date() }}, date_trunc({{ btmh_run_date() }}, month), day) + 1) as bignumeric)
    ) as days_factor
),
...
nx as (
  ...
  from {{ ref('f_nhap_xuat') }} f
  left join {{ ref('d_hach_toan') }} dm
...
{% if is_incremental() %}
having Thang = date_trunc({{ btmh_run_date() }}, month)
{% endif %}
