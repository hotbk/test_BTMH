{{
  config(
    ...
    cluster_by=['Nguon', 'Ma_Kho', 'Nhom_hang']
  )
}}

{% set src_schema = (var('d_hang_schema', 'stg_augges_225') | lower) %}
{% set full_snapshot = var('r_ton_kho_ngay_full_snapshot', false) %}
{% set snapshot_start_date = var('snapshot_start_date', none) %}
{% set snapshot_end_date = var('snapshot_end_date', none) %}

{#
  Kho filter: by default we only include store/selected warehouses.
  If you need SX/other warehouses, override these vars:
    - ton_kho_kho_prefixes: list of 2-char prefixes (e.g. ['CH','ST','KM','GH','SX'])
    - ton_kho_kho_codes: list of exact Ma_Kho codes (e.g. ['FS01', ...])

  To disable filtering entirely, set ton_kho_apply_kho_filter=false.
#}
  ...
  bounds as (
    select
      {% if snapshot_start_date %}
        cast('{{ snapshot_start_date }}' as date)
      {% else %}
        coalesce(
          (select min(date(ngay)) from {{ source('dmt', 'f_ton_kho') }}),
          {{ btmh_run_date() }}
        )
      {% endif %} as start_date,
      {% if snapshot_end_date %}
        cast('{{ snapshot_end_date }}' as date)
      {% else %}
        coalesce(
          (select max(date(ngay)) from {{ source('dmt', 'f_ton_kho') }}),
          {{ btmh_run_date() }}
        )
      {% endif %} as end_date
  ),
...
params as (
  ...
  {% else %}
    select {{ btmh_run_date() }} as p_date
  {% endif %}
),
...
joined as (
  ...
  left join {{ ref('d_nhom_ma_mau') }} nmm
...
{% if is_incremental() and not full_snapshot %}
