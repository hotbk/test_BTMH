{{
  config(
    ...
    cluster_by=['company_code', 'warehouse_id']
  )
}}
...
,cogs as (
    ...
    left join {{ source('stg_augges_225', 'dmnx') }} dnx 
        on nx.ID_Nx = dnx.ID
...
,coc_chua_tra as (
    ...
    from dat_coc dc left join {{ ref('d_hang') }} h on dc.ID_Hang = cast(h.ID as string)
                    left join {{ ref('d_nhom') }} nh on h.ID_Nhom = nh.ID
...
,coc_da_tra as (
    ...
    from doanh_thu dt left join coc_inf dc on dt.Sub_ID = dc.ID and dt.Ma_Cong_Ty = dc.Nguon
                      left join {{ ref('d_hang') }} h on dt.ID_Hang = cast(h.ID as string)
                      left join {{ ref('d_nhom') }} nh on h.ID_Nhom = nh.ID
...
,don_tra_ngay as (
    ...
    from doanh_thu dt left join {{ ref('d_hang') }} h on dt.ID_Hang = cast(h.ID as string)
                      left join {{ ref('d_nhom') }} nh on h.ID_Nhom = nh.ID
...
,don_b2b as (
    select concat(Loai_Chung_Tu, '_', ID_Phieu_Ban, '_4') as order_id
          ...
          ,'Đơn B2B' as type
    from {{ ref('f_b2b') }} dt left join {{ ref('d_hang') }} h on dt.ID_Hang = h.ID
                      left join {{ ref('d_nhom') }} nh on h.ID_Nhom = nh.ID
