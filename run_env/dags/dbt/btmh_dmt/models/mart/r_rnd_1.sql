{{
  config(
    ...
    cluster_by=['MA_CONG_TY', 'KENH_BAN', 'NHOM_HANG']
  )
}}
...
with params as (
  select
    {{ btmh_run_date() }} as report_date,
    date('{{ var("rnd_backfill_start_date", "2024-01-01") }}') as backfill_start_date
),
