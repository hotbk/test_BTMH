{{
  config(
    ...
    partition_by={"field": "ngay_bao_cao", "data_type": "date", "granularity": "month"},
    cluster_by=['kenh', 'chi_tieu', 'danh_muc_san_pham', 'dong_san_pham']
  )
}}
...
with params as (
  select
    ...
    extract(year from {{ btmh_run_date() }}) as curr_year,
    extract(month from {{ btmh_run_date() }}) as curr_month,
    extract(year from date_sub({{ btmh_run_date() }}, interval 1 month)) as prev_year,
    extract(month from date_sub({{ btmh_run_date() }}, interval 1 month)) as prev_month
),

src as (
  select
    *
  from {{ source('ggs', 'ggs_mkt') }}
