{{
  config(
    ...
    partition_by={"field": "Ngay_Chung_Tu", "data_type": "date", "granularity": "month"},
    cluster_by=['Loai_Chung_Tu', 'Ma_Kho', 'Ma_Khach_Hang']
  )
}}
...
{% if var('btmh_snapshot_append', false) or is_incremental() %}
where date(Ngay) >= p.start_month
  and date(Ngay) < p.end_excl
{% endif %}
