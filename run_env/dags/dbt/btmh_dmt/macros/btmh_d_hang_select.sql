{% macro btmh_d_hang_select(src_schema) %}

{#
  Returns a SELECT statement that matches the existing `dim/d_hang.sql` output,
  parameterized by `src_schema` (dbt source name).

  Usage:
    {{ btmh_d_hang_select('stg_augges_225') }}
#}

with
h as (
	select *
	from {{ source(src_schema | lower, 'dmh') }}
),

nmm as (
	select *
	from {{ source(src_schema | lower, 'dmnmm') }}
),

nh as (
	select *
	from {{ source(src_schema | lower, 'dmnh') }}
),

vbmm as (
	select *
	from {{ source(src_schema | lower, 'dmvbmm') }}
),
...
vbtg as (
	select *
	from {{ source(src_schema | lower, 'dmvbtg') }}
),

qttg as (
	select *
	from {{ source(src_schema | lower, 'dmqttg') }}
),

dth as (
	select *
	from {{ source(src_schema | lower, 'dmdt') }}
),
...
tt as (
	select
		Ty_Gia
	from {{ source(src_schema | lower, 'dmtt') }}
	where Ma_Tt = 'USD'
